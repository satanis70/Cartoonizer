...
//        val model = WhiteboxCartoonGanInt8.newInstance(requireContext())
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            ...
            R.id.action_share -> shareCartoon()
        }
        return super.onOptionsItemSelected(item)
    }

    private fun shareCartoon() {
        val image: Bitmap? = BitmapFactory.decodeFile(filePath)
        val share: Intent = Intent(Intent.ACTION_SEND)
        share.type = "image/*"
        share.putExtra(Intent.EXTRA_STREAM, getImageUrl(shareBitmap!!))
        startActivity(Intent.createChooser(share, ""))
    }

    fun getImageUrl(bitmap: Bitmap): Uri?{
